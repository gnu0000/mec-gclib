...
////            (LONG) TLMemSize (TypPtrBase (ps->ptlst));
////new	      l = (LONG) TLMemSize (ps->ptlst);
//
...
//         uLocalOffset += MemStkSize (ps->uTypeVal, ps->psStruct);
         uLocalOffset += TLMemStkSize (ps->ptlst);
//         }
      }
   return uLocalOffset;
   }
...
static void GenExpr (PNODE pStmt)
   ...
   if (pStmt)
      {
      GenExpression (pStmt);
      AddOpS (OP_DSP, 0, TLMemRefStkSize (pStmt->ptlst));
      }
   }
...
static void PushSwitch ()
   ...
   pSwitch = (PSWITCH)calloc (1, sizeof (SWITCH));
...
static void GenCase (PNODE pnode)
   ...
   pCase = (PCASE)calloc (1, sizeof (CASE));
...
static void GenSwitch (PNODE pStmt)
   ...
   for (uCount=0, pTmp=pCase; pTmp; pTmp=pTmp->next)
      uCount++;
   AddShort ((SHORT)uCount);
   AddShort ((SHORT)(!!CurrentSwitch()->pDefault));

   for (pTmp=pCase; pTmp; pTmp=pTmp->next)
      {
      AddWord (pTmp->lVal);
      AddShort ((SHORT)LabPos(pTmp->pLabel));
      }
...
static void GenDeclarationList (PNODE pDLST)
   ...
   for (pVar = pDLST->pn2; pVar; pVar = pVar->next)
      if (pInit = pVar->pn2)
			{
         GenExpression (pInit);
//	      AddOpS (OP_DSP, 0, MemStkSize (pInit->uTypeVal, FindStructType (pInit, 0))); // GNU1200
	      AddOpS (OP_DSP, 0, TLMemStkSize (pInit->ptlst));
			}
   }
...
static void GenFnSuffix (UINT uLocalSize, PSYM ps)
   ...
   if (TLGetType(ps->ptlst) == DATATYPE_VOID)       // void fn type - no return value
      {
   ...
   else if (TLGetType(ps->ptlst) == DATATYPE_STRUCT)// return any # of bytes   
      {
      pE = LabForward ();
...
//      AddOpW (OP_PUSHI, 0, MemSize (ps->uTypeVal, ps->psStruct));
      AddOpW (OP_PUSHI, 0, TLMemSize (ps->ptlst));
...
//      AddOp  (OP_POPLS, ps->uTypeVal); // pop return val to return variable
      AddOp  (OP_POPLS, TLGetType(ps->ptlst)); // pop return val to return variable
      LabSet (pE);                     // non return statements go to here

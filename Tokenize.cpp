...
static FILE *_PushFileA (FILE* fp, PSZ pszFile, int iLine)
	{
   PFINFO pFInfo;
   ...
   pFInfo = (PFINFO)calloc (sizeof (FINFO), 1);
...
static FILE *PushFile (PSZ pszFile)
   ...
   if (!(fp = fopen (pszFile, "rt")))
	...
	return _PushFileA (fp, pszFile, 0);
   }
...
/*
 * Similar to TokInit but used when the file is already open
 * fp			- The open file handle
 * pszFile	- The Name of the file
 * iLine		- The current line number
 * no error return
 */

...
#define STOP_ON_SECTION_BREAK

/***************************************************************************/

int b_getc (void)
	{
#if defined (STOP_ON_SECTION_BREAK)

   int c = f_getc (pINFILE->fp);
	if (c != '\n')
		return c;
   c = f_getc (pINFILE->fp);
   f_ungetc (c, pINFILE->fp);
	return (c == '[' ? EOF : '\n');
#else
...
static PDEF _addDefine (PDEF pTree, PSZ pszDef)
   ...
   if (!pTree)
      {
      pTree = (PDEF)calloc (1, sizeof (DEF));

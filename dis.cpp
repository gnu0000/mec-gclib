...
static PSZ   pszGLOBALS = NULL; // globals area malloced at file load
static PSZ   pszCODE    = NULL; // assigned to pszCode of current fn ???
static PLONG plSTACK    = NULL; // malloced at startup

static INT  iSF = 0;           // stack frame register
static INT  iSP = 0;           // stack pointer
static INT  iIP = 0;           // instruction ptr

static UCHAR cOP;
static UCHAR cOPKIND;
static UCHAR cOPTYP;
...
static PFUNC CodNewFunction (void)
   {
   PFUNC pFunc;

   pFunc = (PFUNC)calloc (sizeof (FUNC), 1);
   return pFunc;
   }
 ...
 * this block
 */
static void LoadGlobals (FILE *fp)
   {
   UINT uSize;
...
static void LoadFunction (FILE *fp)
   {
   PFUNC pFunc;
...
void DumpProgram (PSZ pszFile, PLONG plStack)
   ...
   plSTACK = (plStack ? plStack : (long*)calloc (STACK_SIZE, sizeof (LONG)));

	printf ("Program Module: %s\n", pszFile);
...
static int VMCleanup (void)
   {
   return 0;
   }


static void PByte (void)
   {
   sprintf (sz1, "%2.2x", NextByte ());
   }

static void PShort(void)
   {
   sprintf (sz1, "%4.4x", NextShort ());
   }

static void POff(void)
   {
   INT i;
...
static void PrintBytes (PSZ pszCODE, PINT piAddr, PINT piLen, INT iMaxLen)
   {
   INT i;
   ...
   for (i=0; i<iMaxLen; i++)
      ...
      printf ("%2.2x ", (UINT)(UCHAR)pszCODE[(*piAddr)++]);
      (*piLen)--;
      }
   }
 ...
 *
 */
static void PrintOp (INT iAddr, UINT uLen, PSZ pszOp, PSZ pszOper)
   {
   BOOL  bSwitch;
   PLONG pLong;
   PINT  pJump;

   bSwitch = ((UCHAR)pszCODE[iAddr] == OP_SWITCH);

   /*--- print Address ---*/
   printf ("[%4.4x] ", iAddr);

   /*--- print bytes ---*/
   printf ("%2.2x ", (UCHAR)pszCODE[iAddr++]);       // opcode
 ...
 */
static void DumpFunction (PFUNC pFunc)
   {
   INT iAddr;
   PSZ p1;
   ...
   for (iIP = 0; iIP < (INT)pFunc->uSize;)
      ...
      *sz1= '\0';
      uOpLen = 0;
      switch ((UCHAR)pFunc->pszCode[iAddr])
         {
...
//         case OP_DECG + M_FLOAT:   p1 = "DECG[F]   "; PAddr();    break;
         ...
         case OP_INC + M_BYTE:     p1 = "INC[B]    ";    			break;
         case OP_INC + M_SHORT:    p1 = "INC[S]    ";    			break;
         case OP_INC + M_WORD:     p1 = "INC[W]    ";    			break;
         case OP_INC + M_FLOAT:    p1 = "INC[F]    ";    			break;
         case OP_DEC + M_BYTE:     p1 = "DEC[B]    ";    			break;
         case OP_DEC + M_WORD:     p1 = "DEC[W]    ";    			break;
         case OP_DEC + M_FLOAT:    p1 = "DEC[F]    ";    			break;
         ...
         case OP_SHL3:             p1 = "SHL3      ";             break;
			...
			case OP_HGET:				  p1 = "HGET      "; PWord();    break;
 ...
 *
 */
static void DumpGlobals (PSZ pszGlobals, UINT uSize)
   {
   UINT i, j;
   ...
   for (i=0; i<uSize; i += 16)
      ...
      for (j=0; j<16; j++)
         ...
         if (i+j >= uSize)
            ...
            printf ("%2.2x ", (UCHAR)pszGlobals [i+j]);
         }
      printf ("   ");
      for (j=0; j<16; j++)
         {

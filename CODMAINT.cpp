...
static PFUNC _codFreeAllFunctions (PFUNC pfnTree)
	{
	if (!pfnTree)
		return NULL;
	pfnTree->left  = _codFreeAllFunctions (pfnTree->left );
	pfnTree->right = _codFreeAllFunctions (pfnTree->right);

   pfnTree->pszCode = (PSZ)MemFreeData (pfnTree->pszCode);
   pfnTree->pszName = (PSZ)MemFreeData (pfnTree->pszName);
	free (pfnTree);
	return NULL;
	}

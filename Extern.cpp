...
void AddFn (PVOID pFn, UINT uRetType, UINT uTag, UINT uCConv)
   {
   PFNOD pfnod;

   pfnod = (PFNOD)calloc (1, sizeof (FNOD));
   pfnod->pfn   = pFn;
...
static PFNOD _freeAllFn (PFNOD pTree)
	{
	if (!pTree)
		return NULL;

	pTree->left  = _freeAllFn (pTree->left );
	pTree->right = _freeAllFn (pTree->right);
	free (pTree);
	return NULL;
	}
 ...
 *
 * The 32 bit run time executable must be compiled using
 ...
 */
static void C_Call (PVOID pfn, UINT uRetType, UINT uCConv)
   {
   ULONG  w, lTypes;
...
//      pmt = (PMONOT)(PVOID)(plSTACK + iGStkIdx);
//
//      switch (iTyp)
//         {
//         case M_BYTE:
...
BOOL HandleInternalCall (UINT uTag)
   ...
   if (!(pfnod = (PFNOD)FindFn (uTag)))
         Error ("Internal Function not implemented: %d", uTag);

   if (uTag >= 32000) // special GNU functions
      ((INPROC)(pfnod->pfn))();
   else
      C_Call (pfnod->pfn, pfnod->uRetType, pfnod->uCConv); // C functions
   return TRUE;
   }
